      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-1.5 border-b-1.5 border-sumi/10 dark:border-white/10 bg-sumi/[0.02] dark:bg-white/[0.02] select-none">
        ...
        <div className="flex items-center gap-1 font-mono text-xs">
          <button
            ...
            className={`px-2 py-0.5 rounded border-1.5 transition-colors font-medium ${
              activeTab === "trend"
                ? "border-sumi dark:border-paper bg-sumi text-paper dark:bg-paper dark:text-sumi"
                : "border-transparent text-sumi/60 dark:text-paper/60 hover:text-sumi dark:hover:text-paper"
            }`}
          >
            Trend
          </button>
        ...
        <div className="flex items-center gap-2 font-mono text-[11px]">
          ...
          {activeTab === "trend" && (
            <div className="flex items-center gap-1 pr-1.5 border-r border-sumi/10 dark:border-white/10">
              <button
                onClick={() => setShowAo5((v) => !v)}
                title="Toggle ao5 line"
                className={`px-1.5 py-0.5 rounded border-1.5 transition-colors font-semibold ${
                  showAo5
          ...
          <div className="flex items-center rounded border-1.5 border-sumi/10 dark:border-white/10 p-0.5 bg-paper dark:bg-paper-dark">
            {(["50", "100", "all"] as RangeType[]).map((r) => (
              <button
                ...
                className={`px-1.5 py-0.2 rounded transition-colors uppercase ${
                  rangeFilter === r
